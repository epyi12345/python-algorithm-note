...
- Korean page synchronization: when adding or substantially editing an English documentation page, also create or update the matching Korean `.ko.md` page in the same task when practical.
  - Example: `docs/learning/prefix-sum.md` -> `docs/learning/prefix-sum.ko.md`
  - Example: `docs/practice/0004-target-sales-window.md` -> `docs/practice/0004-target-sales-window.ko.md`
- Plan first for large changes: if a task is expected to change many files, do not start editing immediately. First provide a plan that splits the work into smaller PRs.
- Small PR rule: keep each PR focused on one logical area. Do not combine unrelated learning pages, practice problems, workbench JavaScript, CSS, and deployment configuration in one PR.
- Diff guard: before committing or opening a PR, check the changed file list. If unrelated files are present, revert them or stop and report the issue.
- Old branch rule: do not reuse old conflicted branches. Start from the latest main branch or a clean branch based on the current repository state.
- No-op rule: if there is nothing to change, do not create a commit or PR.
- Remote handling: if the local checkout has no `origin` remote, do not proceed with a workflow that depends on `origin/main`. Report the issue and ask for a clean checkout, or configure the remote only if the environment supports it.
